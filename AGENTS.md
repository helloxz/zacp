...
- **Agent 启动超时（`session.start_timeout`，默认 30s）**：`service.CreateSession` 将「启动 agent + ACP 握手 + 首次 session/new」整段包入 `context.WithTimeout`（0 禁用）；agent 命令/参数配置错误导致进程挂起（如进入非 ACP 模式）时 30s 内必返回明确错误（`start agent 'x' timed out after 30s`），manager 在 Initialize 失败路径 kill 子进程防僵尸；配置项写入 `internal/config` 与两份 `config.example.toml`（`backend/configs/` + `backend/internal/web/` embed 副本，须同步）
